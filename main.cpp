...
int main(){
	...
	if(!bulletT.loadFromFile("resources/sprites/bullet.png")){ ///VER D�NDE PONERLO EN LAS HPP/CPP
		cout<<"Error al cargar la textura\n";
	}
	if(!bulletMobsT.loadFromFile("resources/sprites/bulletMobs.png")){ ///VER D�NDE PONERLO EN LAS HPP/CPP
		cout<<"Error al cargar la textura\n";
	}
	...
	sf::RenderWindow window(sf::VideoMode(1000,700),"Last Stand"); /// TAMA�O DE LA VENTANA
	...
	while(window.isOpen()){ /// BUCLE PRINCIPAL DEL JUEGO
		...
		Ship *ship  = new Ship(110,350,ShipT); /// INICIALIZACI�N. ASIGNACI�N DINAMICA DE MEMORIA. AS� DESPU�S ELIMINA DE MEMORIA EL OBJETO SHIP, Y CREA UNO NUEVO.
		Parallax *parallax = new Parallax(); /// INICIALIZACI�N
		initiated=false; /// INICIALIZACI�N. TODAVIA NO INICIO.
		uis.Initiated(false); /// INICIALIZACI�N
		while(true){ /// EJECUCION DEL JUEGO
            ...
            for(int i = 0;i < bulletsPlayer.size(); i++){ /// ELIMINA EL PROYECTIL JUGADOR AL TOCAR UN OBST � SALGA DE LA PANTALLA.
			...
			if(!sf::Mouse::isButtonPressed(sf::Mouse::Left)){ /// SI NO EST� PRESIONANDO, VUELVE A FALSO
				pressed=false;
			}
